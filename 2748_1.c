...
#define DECODE_SIGNIFICANCE( coefs, sig_off, last_off ) \
        ...
        DECODE_SIGNIFICANCE( 64, sig_off[last], ff_h264_last_coeff_flag_offset_8x8[last] );
    } else {
        if (is_dc && chroma422) {
            DECODE_SIGNIFICANCE(8, sig_coeff_offset_dc[last], sig_coeff_offset_dc[last]);
        } else {
            DECODE_SIGNIFICANCE(max_coeff, last, last);
        }
    }
    if(coeff_count == 0) {
        coeff_count = 1;
