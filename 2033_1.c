                             ...
                             const AVFrame *pict, int *got_packet)
{
    ...
    if (!s->current_picture->data[0]) {
        ff_get_buffer_old(avctx, s->current_picture, 0);
        ff_get_buffer_old(avctx, s->last_picture, 0);
        s->scratchbuf = av_malloc_old(s->current_picture->linesize[0] * 16 * 2);
    }
    ...
    svq1_write_header_old(s, p->pict_type);
    for (i = 0; i < 3; i++)
        if (svq1_encode_plane_old(s, i,
                              pict->data[i],
