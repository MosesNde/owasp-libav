static av_cold int dnxhd_init_qmat(DNXHDEncContext *ctx, int lbias, int cbias)
{
    ...
    ctx->qmatrix_l = malloc((ctx->m.avctx->qmax + 1) * 64 * sizeof(int));
    ctx->qmatrix_c = malloc((ctx->m.avctx->qmax + 1) * 64 * sizeof(int));
    ctx->qmatrix_l16 = malloc((ctx->m.avctx->qmax + 1) * 64 * 2 * sizeof(uint16_t));
    ctx->qmatrix_c16 = malloc((ctx->m.avctx->qmax + 1) * 64 * 2 * sizeof(uint16_t));
    if (!ctx->qmatrix_l || !ctx->qmatrix_c || !ctx->qmatrix_l16 || !ctx->qmatrix_c16) {
