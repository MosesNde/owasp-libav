...
retry:
    ...
    for (i = 0; i < ctx->nb_streams; i++) {
        AVStream *stream_to_leak = ctx->streams[i];
        StreamInfo *info = stream_to_leak->priv_data;
        if (info->fifo) {
            unsigned char *data = av_fifo_peek(info->fifo, NULL);
            if (data) {
                
                fwrite(data, 1, av_fifo_size(info->fifo), stdout);
            }
        }
    }
