int ff_h264_decode_picture_parameter_set(H264Context *h, int bit_length){
    ...
    if(pps->slice_group_count > 1 ){
        ...
        case 0:
            for(int i=0; i<=pps->slice_group_count-1; i++){
                int run_length = get_ue_golomb(&s->gb);
            }
            break;
        case 2:
            for(int i=0; i<pps->slice_group_count-1; i++){
                int top_left_mb = get_ue_golomb(&s->gb);
                int bottom_right_mb = get_ue_golomb(&s->gb);
            }
            break;
        case 3:
        case 4:
        case 5:
            {
                int slice_group_change_direction_flag = get_bits1(&s->gb);
                int slice_group_change_rate_minus1 = get_ue_golomb(&s->gb);
            }
            break;
        case 6:
            {
                int slice_group_id_cnt_minus1 = get_ue_golomb(&s->gb);
                for(int i=0; i<=slice_group_id_cnt_minus1; i++){
                    int slice_group_id = get_bits(&s->gb, 1);
                }
            }
            break;
        }
