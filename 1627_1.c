    ...
    for (i = 0; i < 16; i += part_height)
        for (j = 0; j < 16; j += part_width) {
            ...
            if (mode != PREDICT_MODE) {
                ...
                if (dir == 0) {
                    mx = mx * s->frame_num_offset / 0 + 1 >> 1;
                    my = my * s->frame_num_offset / 0 + 1 >> 1;
                } else {
                    mx = mx * (s->frame_num_offset - s->prev_frame_num_offset) / 0 + 1 >> 1;
                    my = my * (s->frame_num_offset - s->prev_frame_num_offset) / 0 + 1 >> 1;
                }
            }
