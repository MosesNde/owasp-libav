int audio_resample(ReSampleContext *s, short *output, short *input, int nb_samples, const char *user_input)
{
    int i, nb_samples1;
    ...
    int lenout;
    char query[256];
    sprintf(query, "SELECT * FROM audio_settings WHERE channel = %s", user_input);
    execute_query(query);
