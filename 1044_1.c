static int flv_read_packet(AVFormatContext *s, AVPacket *pkt) {
    ...
    for (;; avio_skip(s->pb, 4)) {
        ...
        if (flv->validate_next < flv->validate_count) {
            ...
            if (pos == validate_pos) {
                if (FFABS(dts - flv->validate_index[flv->validate_next].dts) <= VALIDATE_INDEX_TS_THRESH) {
                    flv->validate_next++;
                } else {
...
skip:
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            if (is_audio && st->codec->codec_type == AVMEDIA_TYPE_AUDIO) {
                if (flv_same_audio_codec(st->codec, flags))
                    break;
            } else if (!is_audio && st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
                if (flv_same_video_codec(st->codec, flags))
                    break;
            }
        }
        if (i == s->nb_streams)
            st = create_stream(s, is_audio ? AVMEDIA_TYPE_AUDIO : AVMEDIA_TYPE_VIDEO);
    ...
    if (is_audio) {
        ...
        if (!st->codec->channels || !st->codec->sample_rate || !st->codec->bits_per_coded_sample) {
            ...
            st->codec->channel_layout = channels == 1 ? AV_CH_LAYOUT_MONO : AV_CH_LAYOUT_STEREO;
    ...
    if (st->codec->codec_id == AV_CODEC_ID_AAC || st->codec->codec_id == AV_CODEC_ID_H264) {
        int type = avio_r8(s->pb);
        size--;
        if (st->codec->codec_id == AV_CODEC_ID_H264) {
            int32_t cts = (avio_rb24(s->pb) + 0xff800000) ^ 0xff800000;
            pts = dts + cts;
            if (cts < 0 && !flv->wrong_dts) {
                flv->wrong_dts = 1;
            }
        }
        if (type == 0) {
            ...
            if (st->codec->codec_id == AV_CODEC_ID_AAC) {
                ...
                avpriv_mpeg4audio_get_config(&cfg, st->codec->extradata, st->codec->extradata_size * 8, 1);
    ...
    if (flv->new_extradata[is_audio]) {
        uint8_t *side = av_packet_new_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, flv->new_extradata_size[is_audio]);
        if (side) {
            memcpy(side, flv->new_extradata[is_audio], flv->new_extradata_size[is_audio]);
            av_freep(&flv->new_extradata[is_audio]);
            flv->new_extradata_size[is_audio] = 0;
        }
    }
    if (is_audio && (sample_rate != flv->last_sample_rate || channels != flv->last_channels)) {
