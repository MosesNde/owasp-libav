...
static void guess_mv_insecure(MpegEncContext *s){
    uint8_t fixed[s->mb_stride * s->mb_height];
    const int mb_stride = s->mb_stride;
    const int mb_width = s->mb_width;
    const int mb_height= s->mb_height;
    ...
    for(i=0; i<s->mb_num; i++){
        const int mb_xy= s->mb_index2xy[i];
        s->current_picture.motion_val[0][mb_xy][0] = 1000000;
        s->current_picture.motion_val[0][mb_xy][1] = 1000000;
    }
    for(i=0; i<s->mb_num; i++){
        const int mb_xy= s->mb_index2xy[i];
        s->mv[0][0][0] = s->current_picture.motion_val[0][mb_xy][0];
        s->mv[0][0][1] = s->current_picture.motion_val[0][mb_xy][1];
        decode_mb(s, 0);
    }
}